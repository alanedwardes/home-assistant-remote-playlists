...
class M3uParser:
    ...
    def _check_streams_status(self):
        if self._check_live and len(self._streams_info) > 0:
            ...
            coros = (
                self._check_status(index) for index in range(len(self._streams_info))
            )
    ...
    async def _parse_line(self, line_num: int):
        ...
        try:
            for i in [1, 2]:
                ...
                if self._lines[line_num + i] and (
                    is_acestream or is_valid_url(self._lines[line_num + i])
                ):
    ...
    def _get_m3u_content(streams_info: list) -> str:
        ...
        for stream_info in streams_info:
            ...
            if (
                stream_info.get("country") != None
                and stream_info["country"].get("code") != None
            ):
                line += ' tvg-country="{}"'.format(stream_info["country"]["code"])
            if (
                stream_info.get("language") != None
                and stream_info["language"].get("name") != None
            ):
    ...
    def parse_m3u(
        self, path: str, check_live: bool = True, enforce_schema: bool = True
    ):
    ...
    async def parse_m3u_content(
        self, content: str, check_live: bool = True, enforce_schema: bool = True
    ):
        """Parses the content of local file/URL.
            ...
            self._streams_info = [
                {
                    ...
                    "country": {
                        "code": stream_info.get("country_code"),
                        "name": stream_info.get("country_name"),
                    },
                    "language": {
                        "code": stream_info.get("language_code"),
                        "name": stream_info.get("language_name"),
                    },
        ...
        self._streams_info = [
            {
                ...
                "country": {
                    "code": get_value(row, "country_code"),
                    "name": get_value(row, "country_name"),
                },
                "language": {
                    "code": get_value(row, "language_code"),
                    "name": get_value(row, "language_name"),
                },
